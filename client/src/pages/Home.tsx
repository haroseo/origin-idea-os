...
export default function Home() {
  ...
  const fontUpload = trpc.origin.font.upload.useMutation({
    onSuccess: () => toast.success("손글씨 원본을 브랜드 금고에 저장했습니다."),
    onError: (error) => toast.error(error.message),
  });
  ...
  return (
    <DashboardLayout>
      <div className="origin-page">
        ...
        {page === "handfont" ? <HandfontView preview={fontPreview} uploading={fontUpload.isPending} onFile={handleFontFile} /> : null}
...
function HandfontView({ preview, uploading, onFile }: { preview: string | null; uploading: boolean; onFile: (event: ChangeEvent<HTMLInputElement>) => void }) { const glyphs = ["가", "나", "다", "라", "A", "B", "C", "1", "2", "3"]; return <div className="origin-workspace"><article className="origin-tool-card"><h2 className="origin-tool-title">Handfont Lab</h2><p className="origin-tool-description">글자 시트를 올리면 글리프 단위로 정리하고, 조합 미리보기와 폰트 결과물 준비까지 한 흐름으로 이어갑니다.</p><label className="origin-upload-zone">{preview ? <img src={preview} alt="업로드한 손글씨 시트" /> : null}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={onFile} /><div className="origin-upload-copy"><i className="origin-upload-icon">{uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}</i><strong>{uploading ? "원본을 금고에 보관하는 중" : preview ? "손글씨 시트를 확인했습니다" : "손글씨 시트를 올려주세요"}</strong><span>PNG · JPG · WEBP · 최대 5MB<br />권장: 선명한 흰 배경의 글자 칸</span></div></label><div className="origin-section-header mt-6"><h3 className="origin-section-title">Glyph separation</h3><span className="origin-section-caption">자동 분리 미리보기</span></div><div className="origin-glyph-strip">{glyphs.map((glyph) => <span className="origin-glyph" key={glyph}>{glyph}</span>)}</div><div className="origin-font-preview"><p>기록하는 순간, 나만의 언어가 됩니다.</p></div><div className="origin-tool-actions"><span className="origin-tool-note">벡터화·조합 검수 단계 후 TTF / WOFF2 내보내기가 활성화됩니다.</span><button className="origin-secondary-button" onClick={() => toast.message("손글씨 원본과 분리 글리프를 저장했습니다. 벡터화·폰트 바이너리 생성은 다음 처리 단계입니다.")}><Download className="h-4 w-4" />폰트 내보내기</button></div></article><aside className="origin-side-note"><p className="origin-eyebrow">FONT WORKFLOW</p><h3>손글씨를 폰트로</h3><div className="origin-side-list"><div><i /><span>글자 칸과 획을 인식해 글리프를 분리합니다.</span></div><div><i /><span>한글·영문·숫자 조합으로 품질을 검수합니다.</span></div><div><i /><span>웹용 WOFF2와 설치용 TTF로 내보냅니다.</span></div></div></aside></div>; }
...
function IdeaModal({ brand, pending, onClose, onCreate }: { brand: BrandPreview; pending: boolean; onClose: () => void; onCreate: (payload: { title: string; originalText: string; description?: string; tags: string[] }) => void }) { const [title, setTitle] = useState(""); const [originalText, setOriginalText] = useState(""); const [description, setDescription] = useState(""); const [tags, setTags] = useState(""); return <div className="origin-modal-backdrop" role="dialog" aria-modal="true"><form className="origin-modal" onSubmit={(e) => { e.preventDefault(); onCreate({ title, originalText, description, tags: tags.split(",").map((tag) => tag.trim()).filter(Boolean) }); }}><div className="origin-modal-top"><div><h3>새 Idea ID 기록</h3><p>{brand.name} 금고에 원본과 첫 번째 증거 버전을 만듭니다.</p></div><button type="button" className="origin-icon-button" onClick={onClose}><X className="h-4 w-4" /></button></div><div className="origin-form-grid"><div className="origin-form-field full"><label>제목</label><input className="origin-input" value={title} onChange={(e) => setTitle(e.target.value)} required minLength={2} /></div><div className="origin-form-field full"><label>원본 텍스트</label><textarea className="origin-textarea" value={originalText} onChange={(e) => setOriginalText(e.target.value)} required minLength={8} /></div><div className="origin-form-field full"><label>설명 (선택)</label><textarea className="origin-textarea min-h-20" value={description} onChange={(e) => setDescription(e.target.value)} /></div><div className="origin-form-field full"><label>태그 (쉼표로 구분)</label><input className="origin-input" value={tags} onChange={(e) => setTags(e.target.value)} placeholder="font, format, creator-tool" /></div></div><div className="mt-5 flex justify-end gap-2"><button type="button" className="origin-secondary-button" onClick={onClose}>취소</button><button className="origin-primary-button" disabled={pending}>{pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}원본 잠그기</button></div></form></div>; }
